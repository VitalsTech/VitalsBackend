...
using RoutingService.API.Validators;
...
public static class Program
{
    public static async Task Main(string[] args)
    {
        ...
        {
            c.SwaggerDoc("v1", new OpenApiInfo { Title = "Routing Service", Version = "v1" });
        ...
        builder.Services.AddValidatorsFromAssemblyContaining<TriageCompletedEventValidator>();
        ...
        using (var scope = app.Services.CreateScope())
            scope.ServiceProvider.GetRequiredService<RoutingDbContext>().Database.Migrate();

        await RoutingDataSeeder.SeedAsync(app.Services);
